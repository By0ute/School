...
namespace Testing
{
	...
	double distance(Sign s1, Sign s2)
	{
		...
		while (it1 != (s1.get_datas()).end())
		{
			...
			double dist_alt = pow(abs(v1.get_altitude() - v2.get_altitude()), p);
			double dist_time = pow(abs(v1.get_timeStamp() - v2.get_timeStamp()), p);

			dist_res += dist_x + dist_y + dist_pos + dist_azimut + dist_pression + dist_alt + dist_time;
